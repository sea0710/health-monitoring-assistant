...
Page({
  ...
  loadPatientInfo() {
    ...
    if (patient) {
      ...
      this.setData({ patient: null })
    }
  },

  checkLoginAndPatient() {
    const patient = this.data.patient || getApp().globalData.patientInfo || wx.getStorageSync('patientInfo')

    if (!patient) {
      wx.showModal({
        title: '需要创建患者',
        content: '请先创建患者档案，才能设置复查提醒',
        confirmText: '去创建',
        confirmColor: '#14b8a6',
        success: (res) => {
          if (res.confirm) {
  ...
  async handleQuickCreateBlood() {
    if (!this.checkLoginAndPatient()) return

    const { chemoEndDate, pendingReminders, historyReminders, mode } = this.data
    const patient = this.data.patient || getApp().globalData.patientInfo || wx.getStorageSync('patientInfo')
    if (!chemoEndDate) {
  ...
  async handleQuickCreateOther() {
    ...
    const { lastReviewDate, frequencyIndex, pendingReminders, historyReminders, mode } = this.data
    const patient = this.data.patient || getApp().globalData.patientInfo || wx.getStorageSync('patientInfo')
    if (!lastReviewDate) {
  ...
  async handleConfirmAdd() {
    const { addReminderDate, addReminderTime, addReminderMode } = this.data
    const patient = this.data.patient || getApp().globalData.patientInfo || wx.getStorageSync('patientInfo')
    
    if (!patient) {
      wx.showModal({
        title: '需要创建患者',
        content: '请先创建患者档案，才能添加提醒',
        confirmText: '去创建',
        confirmColor: '#14b8a6',
        success: (res) => {
          if (res.confirm) {
